...
class AccountReport(models.AbstractModel):
    ...
    def _init_filter_branch(self, options, previous_options=None):
        ...
        selected_branches = selected_branch_ids and self.env['account.tag'].browse(selected_branch_ids) or self.env['account.tag']
    ...
    def _set_context(self, options):
        ...
        if options.get('branch_ids'):
            branch_ids = self.env['account.tag'].browse([int(branch) for branch in options.get('branch_ids',[])])
    ...
    def get_report_informations(self, options):
        ...
        if options and options.get('branch'):
            options['selected_branch_ids'] = [self.env['account.tag'].browse(int(branch)).name for branch in options.get('branch_ids',[])]
        return info
